---
title: "Sixth Assignment - CENG574"
...
## Methods and Code (PCA, MDS + clustering + validation)
...
### Clustering Validation
  ...
  First of all, one must know that there are 3 types of validation of clusters. 1. External Indexes, 2. Internal Indexes, 3. Relative Indexes
  
The first one can be used only if the clusters have labels that we can use.

  We will first start with External Indexes to our validations.

#### External Validation
  
```{r, echo=TRUE}
library("ClusterR")
Risk_Flags = k_means_train$Risk_Flag[sample(nrow(k_means_train),250)]
external_rand_result_kmeans = external_validation(Risk_Flags,k2$cluster,method = "adjusted_rand_index",
                                                  summary_stats = TRUE)


```
  Here, from the external validation of the clusters one can deduce that, whether the clusters are correctly collected the values as indicated in the system [9]. 
  
#### Internal Validation

  Now let's continue with second, Internal indexes,

```{r, echo=TRUE}
library("cluster")
library("factoextra")
km.res <- eclust(scaled_k_means_train[sample(nrow(scaled_k_means_train),250),], "kmeans", k = 7, nstart = 25, graph = FALSE)
...
# Dunn index and others
library("clValid")

clmethods <- c("hierarchical", "kmeans")
internals <- clValid(scaled_clustering_train[sample(nrow(scaled_clustering_train),250),], nClust = 2:7,
                     clMethods = clmethods,
                     validation = "internal")

```

Here we used Silhouette index and the index gives us a score in between 1 and 0. 1 means the dataset is well clustered, contrary 0 means there might be clusters that are unnecessary for the dataset. We can also get -1 which means dataset can be misclassified. Average Silhouette width is 0.15, hence close to 0, which indicates that dataset is clustered with too many clusters [10] [14].


  Finally, end validation techniques with Relative indexes.
  
#### Relative Index Validation
  
```{r, echo=TRUE}
library("clv")

# cluster data
agnes.mod <- agnes(scaled_clustering_train[sample(nrow(scaled_clustering_train),250),]) # create cluster tree 
...
cls.scatt <- cls.scatt.data(scaled_clustering_train[sample(nrow(scaled_clustering_train),250),], v.pred, dist="manhattan")

# once computed values use in both functions
...
```
 ...
 Davies in its nature, by the conditions it needs measures how good a clustering scheme is. R_{i,j} needs to be equal or above 0. R_{i,j} == R_{j,i}
and so. Hence at the end, satisfying system will have a low value in davies score which indicates better separation of the clusters and "tightness" of the intracluster.
...
### External Validation Result (Assumed labels exist)

  Here, we see external adjusted rand validation result for kmeans.
  
```{r,echo=TRUE}
# Summary of external

external_rand_result_kmeans

```

  Value of external validation, which is closer to 0 indicates that no pair is classified in the same way under both clusterings. This might be not that accurate since label is only assumed. Internal and Relative Validation Measurements and External Measurement F result, etc. are better representers of the clustering validation.

### Internal and Relative Validation Results (Most important)

  Here, we see internal and relative validation results for hierarchical and kmeans.
...
# another approach to dunn1 and dav
dunn1
davies1

```
 In here we can see best method to use between k-means, and hierarchical, Connectivity, Dunn, Silhouette, Davies results and Average width of  Silhouette. These results are shown for both hierarchical and k-means and from k = 2 to k = 7 clusters [13].
...
## Analysis and Evaluation of Results, and Discussion
  ...
  Our Clustering Validations in general, all of them(Relative, Internal, External) shows what we assumed from the linearly decreasing elbow in K-means analysis, actually non-scaled elbow at K=2 clusters mostly better for clustering. Silhouette gives us really low width scores which indicate clusters are not well clustered, and actually our scores indicate that sample could be assigned to another closest cluster. In the Relative part for Dunn, and Davies (Mostly Davies) indicates that clusters are not tightly bound inside, and not well separated from each other.External part from it's score that is close to 0 shows that pairs are actually classified somewhat randomly since R = Correct classification/all-elements. 
  
  One can discuss that how well these clustering methods for this dataset is defined. Is this data fit to clustering or any other unsupervised learning methods. These topics can be discussed in further talks.
...
## Conclusion
  ...
  From the validation part one can interpret that the data is overclustered with k = 7, and clustering approach that is used in the data even the best method given from the clValid shows us that even that doesn't separate the data well for scaled dataset. [11] [12]
...
[9]https://search.r-project.org/CRAN/refmans/clv/html/Davies_Bouldin.html

[10]http://www.sthda.com/english/wiki/wiki.php?id_contents=7952#silhouette-analysis

[11]https://www.listendata.com/2016/01/validate-cluster-analysis.html

[12]https://www.datanovia.com/en/lessons/cluster-validation-statistics-must-know-methods/

[13] https://www.youtube.com/watch?v=_KKT55JohcU

[14] https://www.youtube.com/watch?v=_b87tfU4vKM